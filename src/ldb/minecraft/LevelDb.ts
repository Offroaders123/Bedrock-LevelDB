...
export default class LevelDb implements IErrorable {
  ...
  parseIndexBytes(
    ...
    const restarts = getUnsignedInteger(
      data[length - 4],
      data[length - 3],
      data[length - 2],
      data[length - 1],
  ...
  parseLogContent(content: Uint8Array, context?: string) {
    ...
    while (index < content.length - 6) {
      ...
      const length = getUnsignedShort(content[index + 4], content[index + 5], true);
  ...
  addValueFromLog(content: Uint8Array, index: number, length: number, context?: string) {
    ...
    while (index <= startIndex + length - 5) {
      ...
      if (index <= content.length) {
        const key = getAsciiStringFromUint8Array(keyBytes);
    ...
    while (index < startIndex + length) {
      ...
      switch (tag.value) {
        case 1: // comparator
          ...
          this.comparator = getAsciiStringFromUint8Array(comparatorBytes);
        ...
        case 5: // compactPointer
          ...
          this.compactPointerStrings.push(getAsciiStringFromUint8Array(compactPointerStrBytes));
        ...
        case 7: // newFile
          ...
          this.newFileSmallest.push(getAsciiStringFromUint8Array(newFileSmallestStrBytes));
          ...
          this.newFileLargest.push(getAsciiStringFromUint8Array(newFileLargestStrBytes));
